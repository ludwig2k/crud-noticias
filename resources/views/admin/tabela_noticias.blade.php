<x-app-layout>
    ...
    Swal.fire({
        icon: 'success',
        title: 'Notícia excluida com sucesso!',
        showConfirmButton: false,
        timer: 5000
    })
    </script>
                            ...
                            @foreach ($noticias as $noticia)
                                <tr>
                                    ...
                                    <td class="px-6 py-4 whitespace-no-wrap">
                                        <a href="{{route('admin.editarNoticia', $noticia->id)}}" class="text-indigo-600 hover:text-indigo-900">Editar</a>
                                        <form class="inline-block" action="{{ route('admin.destroyNoticia', $noticia->id) }}" method="POST" onsubmit="return confirm('Tem certeza que deseja excluir esta notícia?')">
                                            @csrf
                                            @method('DELETE')
